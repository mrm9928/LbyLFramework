...
Data_hist= Inputs.Get("pt/data;1") #Data Histogram
Sig_hist = Inputs.Get("pt/signal;1") #LbyL Signal Histogram
eeBkg_hist = Inputs.Get("pt/ee;1") #QED Background Histogram
CEPbkg_hist = Inputs.Get("pt/cep;1") #CEP Background Histogram
...
StatUnc_contents =[]
BW_contents = []
diffid_contents = ["70.985", "29.406", "11.214", "2.349", "0.077"]
MCcross_contents = []
...
def rapDifCross():
    ...
    for bin in range(0, 5):
        ...
        print ("Differential cross section for average photon momentum[", bin + 1 ,"]:", diffid_contents[bin])
...
def statUncData():
    ...
    for bin in range(0, Data_hist.GetNbinsX()-4):
        content = Data_hist.GetBinContent(4-bin)
        Data_hist_contents.append(content)

        content2 = eeBkg_hist.GetBinContent(4-bin) 
        eeBkg_hist_contents.append(content2)
            
        content3 = CEPbkg_hist.GetBinContent(4-bin) 
