...
def cosplot():
    ...
    difcross_contents=[float(x) for x in cosDifCross.cosDifCross()]
    staterr_contents=[float(i) for i in cosDifCross.statUncData()]
    phreco_contents=[float(i) for i in coserrdif.Err("PH_RECOeta", "PhReco", "PhReco")]
    phiden_contents=[float(i) for i in coserrdif.Err("PH_PIDeta", "PhIden", "PhIden")]
    enresodown_contents=[float(i) for i in coserrdif.Err("EG_RESOLUTION_ALL__1down", "EnResoDown", "EnResoDown")]
    enresoup_contents=[float(i) for i in coserrdif.Err("EG_RESOLUTION_ALL__1up", "EnResoUp", "EnResoUp")]
    enscaleup_contents=[float(i) for i in coserrdif.Err("EG_SCALE_ALL__1down", "EnScaleDown", "EnScaleDown")]
    enscaledown_contents=[float(i) for i in coserrdif.Err("EG_SCALE_ALL__1up", "EnScaleUp", "EnScaleUp")]
    trigdown_contents=[float(i) for i in coserrdif.Err("TRIG__1down", "TrigDown", "TrigDown")]
    trigup_contents=[float(i) for i in  coserrdif.Err("TRIG__1up", "TrigUp", "TrigUp")]
    #MCerr_contents=[float(i) for i in cosDifCross.MCunc()]
    #MCcross_contents=[float(i) for i in cosDifCross.MCcross()]

    edges = array.array('d', [0, 0.25, 0.5, 0.75, 1])
    ...
    # Set axis titles
    ax1.set_ylabel("d\sigma/d|cos(\\theta*)| [nb]", titleoffset=1.7)
    ...
    fig.savefig("~/IAP/Framework/Outputs/diffcross_cosgraph.pdf")
